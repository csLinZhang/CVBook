//此程序示范了如何用opencv来进行相机内参标定，以及如何利用获取的相机内参来进行鱼眼图像的畸变去除。
//该程序有3个main函数，编译时，每次都需要注释到其他两个，而只保留一个main函数
//main函数1：完成对标定板图像的采集
//main函数2：基于main函数1所采集的标定板图像，完成相机内参标定，并将参数文件存储在本地磁盘
//main函数3：从由main函数2所得到的相机内参文件中，读入相机内参，对原始输入视频进行实时去畸变
//同济大学，张林，2023年4月
#include <string>
#include <iostream>
...
//main函数1
//这个main函数完成对标定板图像的采集
//正常进入采集流程后，按q键就会拍摄一张图片，并会在当前图像上进行交叉点检测，如果成功，就会把这张图像保存下来
//按escape键，退出采集过程
int main()
{
	cout << "开始采集标定板图像" << endl;
	//imgDirName，存放所采集的标定板图像
	string imgDirName = "C:\\lin\\CV-Intro\\code\\chapter-10-imaging model and intrinsics calibration\\cameracalib\\cameracalib\\imgs\\";
	
	Mat frame;
	string imgname;
	int frameIndex = 1;

	cv::Size iPatternSize(9, 6); //交叉点的个数，如果是10*7块的标定板，其有效交叉点是9*6个
	vector<Point2f> gCorners;

	//打开相机并设置相机分辨率。相机的序号从0开始。如果是笔记本外接USB相机，一般来说其编号是1，因为笔记本自带摄像头的编号为0
	...
	while (1) 
	{
		...
		if (key == 'q' || key == 'Q') 
		{
			...
			else //将当前帧frame存储在本地磁盘路径imgDirName中
			{
				...
				imgname = imgDirName + to_string(frameIndex++) + ".jpg"; //为当前帧起好名字，存储
...
//main函数2
//这个main函数完成从一组标定板图像中对相机内参进行标定的任务
//相机畸变模型采用鱼眼相机模型，适用于广角鱼眼相机去畸变任务
//int main(int argc, char* argv[])
//{
//	cv::Size iPatternSize(9, 6); //标定板上的交叉点维度，10*7格的标定板，其交叉点为9*6
//
//	string imgDir = "C:\\lin\\CV-Intro\\code\\chapter-10-imaging model and intrinsics calibration\\cameracalib\\cameracalib\\imgs\\";
//	//存储相机内参标定结果的文件
...
//	//gFileNames，存储所有标定板图像文件全名称
//	cv::glob(imgDir, gFileNames);
//	cout << "Load images" << endl;
//	//nImageCount，标定板图像的数目
//	int nImageCount = gFileNames.size();
//	//gCorners，存储一张图像上交叉点坐标信息.
//	vector<Point2f> gCorners;
...
//	Size iSquareSize = Size(51, 51); //
//	vector<vector<Point3f>>  gObjectPoints;
//	gObjectPoints.clear();
//	
//	//gPointsCount,是个向量，其维度为nImageCount，每个元素表示这张图像的交叉点的个数
...
//main函数3
//该main函数应用之前已经获取到的相机内参，完成输入视频的实时去畸变
//int main(int argc, char* argv[]) 
//{
...
//	FileStorage camPramsFile("C:\\lin\\CV-Intro\\code\\chapter-10-imaging model and intrinsics calibration\\cameracalib\\cameracalib\\camParams.xml", FileStorage::READ);
...
//	Mat shrinkedOriFrame, shrinkedUndistortedFrame; //为了便于屏幕显示，我们把帧尺寸缩小一些
//
//	while (1) 
//	{
//		inputVideo >> oriframe;
