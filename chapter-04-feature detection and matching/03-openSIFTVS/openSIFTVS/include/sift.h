...
/** 尺度不变特征点数据结构
 r,c,layerIndex是最后一次精化操作之前的在DoG尺度空间中的位置信息，都为整数。
 也就是说，最后一次偏移量估计是基于该点[r,c,intvl]进行泰勒展开的。
 之所以要记录这个整数位置，是为了后面判断边缘点和构建描述子方便
	*/
struct detection_data
{
  ...
  int octv; //所在组
  int intvl; //所在层
  double subintvl; //亚层，这是为了最终进行尺度插值使用的
  double scl_octv; //组内亚层尺度，注意：每组对应层的尺度都是一样的，2倍关系通过下采样来体现
};
